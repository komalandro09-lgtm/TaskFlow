  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-2xl font-extrabold tracking-tight text-white md:text-3xl">Projects Tracker</h2>
          <p className="text-sm text-slate-400">Organize goals, align team members, and track dashboard progress.</p>
        </div>
        <button
      ...
      <div className="flex flex-col gap-3 rounded-2xl border border-slate-800/80 bg-slate-900/20 p-4 sm:flex-row sm:items-center">
        ...
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            className="rounded-xl border border-slate-800 bg-slate-950/40 py-3 px-4 text-xs text-slate-200 focus:border-brand-500 focus:outline-none cursor-pointer"
          >
      ...
      {filteredProjects.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-slate-800/80 bg-slate-900/10 py-16 text-center">
        ...
        <div className="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3">
          {filteredProjects.map((project) => (
            <div
              ...
              className="relative flex flex-col justify-between rounded-2xl border border-slate-800/80 bg-slate-900/30 p-6 shadow-md cursor-pointer hover:border-slate-700/80 transition-all duration-200"
            >
              {/* Menu and badges */}
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="relative">
                  ...
                  {activeProjectMenu === project.id && (
                    <div className="absolute right-0 z-20 mt-1 w-32 rounded-lg border border-slate-800 bg-slate-950 p-1 shadow-xl">
                      <button
                        onClick={(e) => handleDelete(project.id, e)}
                        className="flex w-full items-center gap-1.5 rounded px-2.5 py-1.5 text-left text-xs font-semibold text-rose-400 hover:bg-slate-900"
                      >
                        <Trash2 size={12} />
              ...
              {/* Dates */}
              <div className="mt-6 flex flex-wrap items-center gap-x-4 gap-y-1.5 border-t border-slate-850 pt-4 text-[10px] text-slate-500 font-semibold">
